...
type DB struct {
	...
	waitDuration int64 // Total time waited for new connections.获取新连接时总计等待的时间

	connector driver.Connector
	// numClosed is an atomic counter which represents a total number of
	// closed connections. Stmt.openStmt checks it before cleaning closed
	// connections in Stmt.css.
	// numClosed是一个原子计数器，记录已关闭的连接总数。
	numClosed uint64

	mu           sync.Mutex // protects following fields
	freeConn     []*driverConn
	connRequests map[uint64]chan connRequest
	nextRequest  uint64 // Next key to use in connRequests.下一个将处理的连接请求
	numOpen      int    // number of opened and pending open connections 打开和挂起的连接数
	// Used to signal the need for new connections
	// a goroutine running connectionOpener() reads on this chan and
	// maybeOpenNewConnections sends on the chan (one send per needed connection)
	// It is closed during db.Close(). The close tells the connectionOpener
	// goroutine to exit.
	// openerCh缓冲新创建连接请求，connectionOpener()从中读取数据创建连接，maybeOpenNewConnections从中写入数据。
	// 当db.Close()关闭的时候它会倍关闭，并通知connectionOpener()退出。
	...
	// 最大的空闲连接数，0表示使用默认值defaultMaxIdleConns（2）
	maxIdle int // zero means defaultMaxIdleConns; negative means 0
	// 最大连接数，0表示不受限制
	maxOpen     int           // <= 0 means unlimited
	maxLifetime time.Duration // maximum amount of time a connection may be reused

	// 清理程序执行chan，只在startCleanerLocked中初始化，只在SetConnMaxLifetime时才会赋值或者db关闭时对该chan进行关闭。
...
// Result, Rows)
// driverConn是对driver.Conn的封装，加入了锁，然后可以用给其它类(Tx, Stmt, Result, Rows)使用。
...
func (db *DB) SetMaxIdleConns(n int) {
	...
	// 如果当前空闲的连接数大于设置的最大空闲连接数，则进行淘汰处理。
...
func (db *DB) SetConnMaxLifetime(d time.Duration) {
	...
	if d > 0 && d < db.maxLifetime && db.cleanerCh != nil {
		// TODO(tfbrother) 此处设计牛逼，db.cleanerCh != nil 表明cleanerCh被初始化了，但cleanerCh里面是否有值不确定
		select {
...
func (db *DB) startCleanerLocked() {
	// 当最大生命周期大于0，且当前打开的连接数大于0，且cleanerCh == nil
	// TODO(tfbrother) 为何要有cleanerCh == nil 这个条件呢？cleanerCh标志connectionCleaner是否已经在执行。
...
func (db *DB) connectionCleaner(d time.Duration) {
	...
	for {
		select {
		case <-t.C:
		case <-db.cleanerCh: // maxLifetime was changed or db was closed.只有SetConnMaxLifetime时才会赋值或者db关闭了。
		}
		...
		// 在上面执行了d = db.maxLifetime，所以此处需要重新对d进行赋值，其实用f := db.maxLifetime更好些
		if d < minInterval {
			d = minInterval
		}
		t.Reset(d)
...
type DBStats struct {
	...
	ConnCreateNum int64 // The total number of connections created due to process life time.
}
...
func (db *DB) Stats() DBStats {
	...
	stats := DBStats{
		...
		ConnCreateNum:     db.connCreateNum,
	}
	return stats
}
...
// 如果存在connRequests并且尚未到达最大连接数限制，则告诉connectionOpener新开连接。
...
func (db *DB) openNewConnection(ctx context.Context) {
	...
	// maybeOpenNewConnctions在往db.openerCh发送之前已经执行了db.numOpen++，所以该函数里面需要
	// 执行db.numOpen--，如果创建连接失败。
...
// putConn把连接放入数据库空闲的池子里面去。（调用putConnDBLocked实现）
// 只有*driverConn.releaseConn方法中调用该函数时resetSession时true
...
// If err != nil, the value of dc is ignored(忽略).
// If err == nil, then dc must not equal nil.
// If a connRequest was fulfilled or the *driverConn was placed in the
// freeConn list, then true is returned, otherwise false is returned.
// 只被openNewConnection和putConn两个方法会调用
func (db *DB) putConnDBLocked(dc *driverConn, err error) bool {
	...
	// 如果有请求排队的情况，则把当前需要放回连接池的请求直接给排队中的第一个
	if c := len(db.connRequests); c > 0 {
		...
		return true
	} else if err == nil && !db.closed { //判断是否需要回收
		// 如果当前设置的最大空闲连接数大于当前空闲连接数，则将连接放入当前空闲连接列表中去
		if db.maxIdleConnsLocked() > len(db.freeConn) {
			db.freeConn = append(db.freeConn, dc)
			db.startCleanerLocked()
			return true
		}
		// TODO(tfbrother)当前空闲连接数超过最大空闲连接数了，按理应该对该连接进行清理，而此处只是增加maxIdleClosed的值
		// 对该连接是何时清理的呢？只有openNewConnection和putConn两个方法里面三处调用了putConnDBLocked，而且参数err == nil的只有
		// 两处，而当putConnDBLocked返回false的时候，外层函数直接关闭了该连接，所以是在外层关闭了该连接，内层进行了db.maxIdleClosed++操作。
		// 代码可读性来说，应该在外层进行db.maxIdleClosed++。比如db.numOpen--就在外层处理的。
		db.maxIdleClosed++
	}
	return false
}
...
type Conn struct {
	...
	// 当关闭的时候，done从0变成1，一旦done变成1，所有的操作将返回ErrConnDone错误
	// 在检查done值的时候，要采用原子操作
	done int32
}
...
// PingContext用于验证数据库连接是否还是alive的。
